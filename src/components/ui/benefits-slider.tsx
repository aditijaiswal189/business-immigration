...
export default function BenefitsSlider({ 
  benefits, 
  region = "this destination",
  defaultActive,
  className = ""
}: BenefitsSliderProps) {
  const [activeBenefit, setActiveBenefit] = React.useState<string>(
    defaultActive || benefits.find(b => b.isActive)?.id || benefits[0]?.id || ""
  );
          ...
          {benefits.map((benefit) => (
            ...
            >
              <div className="space-y-[0.31rem]">
                <h2 className="text-[2.625rem] font-medium leading-[1.2] tracking-[-0.0525rem] uppercase bg-gradient-to-r from-[#95502F] via-[#F5C178] to-[#F5C178] bg-clip-text text-transparent">
                  {benefit.title}
                </h2>
        ...
        <div className="w-full">
          <div className="text-center mb-12">
            ...
            <p className="text-white/80 text-lg">
              Discover the benefits of {region} immigration
            </p>
          </div>
